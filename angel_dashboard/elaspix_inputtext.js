 ...
 *    [iconurl] optional icon that is shown at the right end of the input
 **/
...
function createInputtext(settings) {
    ...
    function inflate() {
        ...
        if (settings.iconurl != undefined) {
            ...
            mIcon = $("<div/>").append($("<img/>").attr("src", settings.iconurl).attr("dragable", "false"));
            mContainer.append(mIcon);
        }
    ...
    function setupCSS() {
        ...
        mContainer.css("position", "relative");//to be used as reference for the Icon
        ...
        mInput.addClass("form-control design-farat");

        if (mIcon != undefined) {
            mIcon.css("position", "absolute");
            mIcon.css("padding", "6px");
            mIcon.css("right", "20px");
        }
    ...
    function setHintClass(aClass) {
        ...
        if (aClass != "text-danger" && aClass != "text-warning" && aClass != "text-success") {//show gray color only if text-danger is not given which forces to show text in red
            //mHint.addClass("text-muted");//gray
            mHint.addClass("design-farat");//gray
        }
