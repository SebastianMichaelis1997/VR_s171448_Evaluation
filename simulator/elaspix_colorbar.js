...
function createColorbar(settings)
{
	...
	function inflate()
	{
		...
		$.each(settings.colors,function(index,color){createEntry(color);});
	...
	function createEntry(aColor)
	{
		...
		inside.on("click touchstart",function(){onClick(aColor);});
		inside.hover(function(){onHoverIn(inside);},function(){onHoverOut(inside);});
